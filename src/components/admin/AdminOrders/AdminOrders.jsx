...
const AdminOrders = () => {
  ...
  const carts = useSelector((state) => state.allCarts).filter(
    (e) => e.status !== "open"
  );
  ...
  const handleDetailClick = async (email, CartId) => {
    const productos = await axios.get("/cart/" + email);
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(send);
    await axios.put("/order", productStatus);
    await axios.post("/sendMail", send);
